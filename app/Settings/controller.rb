...
class SettingsController < Rho::RhoController
  ...
  def login_callback
    ...
    if errCode == 0
      ...
      # We will get this if the RhoConnect server is not functioning.
      if errCode == Rho::RhoError::ERR_REMOTESERVER || errCode == Rho::RhoError::ERR_NETWORK
        @msg = "Cannot log in to the RhoConnect server.  Please restart the application to try again."
        
        if !@msg || @msg.length == 0   
          @msg = Rho::RhoError.new(errCode).message
        end
        
        WebView.navigate( url_for :action => :no_login, :query => {:msg => @msg} ) 
      end
    end  
  end
  ...
  def do_simple_login
    begin
      SyncEngine.login('a', 'a', (url_for :action => :login_callback) )
